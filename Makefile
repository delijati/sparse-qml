...
# 1) run make once to create and update the po/sparse-qml.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/sparse-qml.pot po/en.po
# 3) edit the po file
...
po/sparse-qml.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/sparse-qml.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
